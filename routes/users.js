                   ...
                   if(err){
                       ...
                       if(rows.length >0){
                           if(login_type != phone && rows['login_type']==login_type){
                               res.send(JSON.stringify({success:false,message:"Social Login"}))
                           }else{
                               res.send(JSON.stringify({success:false,message:"Phone existe Déjà ......."}))
                           }
                       }else {
                           ...
                           con.query('insert into conducteur(online,nom,prenom,phone,mdp,statut,login_type,tonotify,creer,statut_licence,statut_nic,statut_vehicule,email) ' +
                              ...
                              function (err,rows,field) {
                                  if(err){
                                      ...
                                      if(rows.affectedRows > 0){
                                          res.send(JSON.stringify({success:true,message:rows.insertId}))
